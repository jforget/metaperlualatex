-- $Id: luatexko-mplib.lua,v 1.3 2011/06/28 01:29:20 nomos Exp $
--
-- written by Dohyun Kim <nomos at ktug org>
-- public domain
--
module('luatexkomplib', package.seeall)
local err, warn, info, log = luatexbase.provides_module({
    name          = "luatexko-mplib",
    version       =  1.1,
    date          = "2013/03/30",
    description   = "Metapost with LuaTeX",
    license	  = "Public Domain",
    comment	  = "derived from luatex-mplib.lua by Hans Hagen",
})

local function term(...)
    texio.write_nl('term', 'luatexko-mplib: ' .. string.format(...))
end
...
local function pdf_startfigure(n,llx,lly,urx,ury)
    -- %s changed into %f by nomos
    tex.sprint(format("\\mplibstarttoPDF{%f}{%f}{%f}{%f}",llx,lly,urx,ury))
end
...
function pdf_textfigure(font,size,text,width,height,depth)
    ...
    -- %s changed into %f by nomos
    tex.sprint(format("\\mplibtextext{%s}{%f}{%s}{%s}{%f}",font,size,text,0,-( 7200/ 7227)/65536*depth))
end
...
function luakosetboxes (data)
    local i = tex.count[14] -- newbox register
    for _,c,_ in string.gmatch(data,"(%A)btex(%A.-%A)etex(%A)") do
        i = i + 1
        c = string.gsub(c,"^%s*(.-)%s*$","%1")
        tex.sprint(format("\\setbox%i\\hbox{%s}",i,c))
    end
end

function luakogetboxes (data)
    local i = tex.count[14] -- the same newbox register
    data = string.gsub(data,"(%A)btex%A.-%Aetex(%A)",
        function(pre,post)
            i = i + 1
            return
                pre ..
                format("textext(%i,%f,%f,%f)",
                    i,
                    tex.box[i] and (tex.box[i].width/factor) or 0,
                    tex.box[i] and (tex.box[i].height/factor) or 0,
                    tex.box[i] and (tex.box[i].depth/factor) or 0) ..
                post
        end)
    return mplibcodepreamble .. data
end

local function luakoputboxes (object)
    local n,tw,th = string.match(object.prescript,
        "%%%%textext:(%d+):([%d%.%+%-]+):([%d%.%+%-]+)")
    if n and tw and th then
        ...
        tex.sprint(format("\\mplibputtextbox{%i}",n))
        pdf_literalcode("Q")
    end
end

--
-- redefine flush function
--
...
--                              if objecttype == "start_bounds" or objecttype == "stop_bounds" then ---- nomos
                                local prescript = object.prescript --++ nomos
                                if prescript and string.find(prescript,"%%%%textext:") then --++ nomos
                                    luakoputboxes(object) --++ nomos
                                elseif objecttype == "start_bounds" or objecttype == "stop_bounds" then --++ nomos
                                -- skip
				...
				local cr = nil --++ nomos
                                if cs and #cs > 0 then
--                                    pdf_literalcode(colorconverter(cs)) ---- nomos
                                        local start,stop = colorconverter(cs) --++ nomos
                                        pdf_literalcode(start) --++ nomos
                                        cr = stop --++ nomos
                                end
                                ...
                                if dl then
--                                    local d = format("[%s] %i d",concat(dl.dashes or {}," "),dl.offset) ---- nomos
				    local d = format("[%s] %i d",table.concat(dl.dashes or {}," "),dl.offset) --++ nomos
                                    if d ~= dashed then
                                        dashed = d
