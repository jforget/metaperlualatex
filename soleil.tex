...
% Formatte un nombre de secondes de diverses manières :
%    n fmt  zero  résultat
% 7384  1    ?    7~384
% 7384  2    0    02:03:04
% 7384  2    1    2:3:4
% 7384  3    0    2~h 3~mn 4~s
% 7384  3    1    02~h 03~mn 04~s
% 7384  4    0    2~h, 3~mn et 4~s
% 7384  4    1    02~h, 03~mn et 04~s
\directlua{
function fmtnb(n, zero)
  if n < 10 and zero == 0 then
    return '0' .. tostring(n)
  else
    return tostring(n)
  end
end
function fmtsec(n, fmt, zero)
  local floor = math.floor;
  local mod   = math.fmod;
  local tilde = string.char(126);
  local ch, sep1, sep2, sep3;
  if fmt == 1 then
    ch = tostring(mod(n, 1000));
    n = floor(n / 1000);
    if n > 0 then
      ch = tostring(n) .. tilde .. ch;
    end
  else
    local s = mod(n, 60);
    ...
    if fmt == 2 then
      sep1, sep2, sep3 = ':', ':', '';
    elseif fmt == 3 then
      sep1, sep2, sep3 = tilde .. 'h ', tilde .. 'mn ', tilde .. 's';
    elseif fmt == 4 then
      sep1, sep2, sep3 = tilde .. 'h, ', tilde .. 'mn et ', tilde .. 's';
    else
      sep1, sep2, sep3 = '?', '?', '?';
    end
    ch = fmtnb(h, zero) .. sep1 .. fmtnb(m, zero) .. sep2 .. fmtnb(s, zero) .. sep3;
  end
  tex.sprint(ch);
end
}
...
\directlua{fmtsec(\veilmidi, 2, 0)} à \directlua{fmtsec(\jourmidi, 2, 0)}, soit \deltamidi{} secondes.
...
passant de \directlua{fmtsec(\veilaube, 2, 0)} à \directlua{fmtsec(\jouraube, 2, 0)},
tandis que le coucher du soleil progresse de :

\formulesoir{}

c'est-à-dire qu'il retarde de \deltasoir~secondes, passant de \directlua{fmtsec(\veilsoir, 2, 0)} à
\directlua{fmtsec(\joursoir, 2, 1)}. Il y a donc une différence, mais qui passe inaperçue, car la
...
solaire retarde de \deltamidi~secondes, passant de \directlua{fmtsec(\veilmidi, 2, 0)} à \directlua{fmtsec(\jourmidi, 2, 0)}.
Le lever du soleil progresse donc de :

\formuleaube{}

c'est-à-dire qu'il retarde de \deltaaube~secondes, passant
de \directlua{fmtsec(\veilaube, 2, 0)} à \directlua{fmtsec(\jouraube, 2, 0)}, tandis que le
coucher du soleil progresse de :

\formulesoir{}

c'est-à-dire que le coucher du soleil retarde de \deltasoir~secondes,
passant de \directlua{fmtsec(\veilsoir, 2, 0)} à \directlua{fmtsec(\joursoir, 2, 0)}.
...
Le 20~mars, le jour dure \directlua{fmtsec(\lgjveilmidisec, 3, 1)}, soit \directlua{fmtsec(\lgjveilmidisec, 1, 0)}~s.
Le 21~mars, il dure \directlua{fmtsec(\lgjjourmidisec, 3, 1)}, soit \directlua{fmtsec(\lgjjourmidisec, 1, 0)}~s
et le 22~mars, il dure \directlua{fmtsec(\lgjlendmidisec, 3, 1)}, soit \directlua{fmtsec(\lgjlendmidisec, 1, 0)}~s.
Cela ne veut pas dire que le 20~mars à 23:59, la durée
du jour est de \directlua{fmtsec(\lgjveilmidisec, 1, 0)}~s et que le 21~mars à 0:01, elle est
brusquement passée à \directlua{fmtsec(\lgjjourmidisec, 1, 0)}~s. La durée varie de façon continue
